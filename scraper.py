...
# E-Mail ist optional, aber DRINGEND empfohlen: OpenAlex stuft Anfragen ohne
# mailto-Parameter in einen sehr stark rate-gelimiteten "common pool" ein
# (das äußert sich als HTTP 429 "Too Many Requests"). Mit hinterlegter Email
# landest du im deutlich schnelleren "polite pool".
#
# WICHTIG auf Streamlit Community Cloud: Secrets, die du im "Secrets"-Tab der
# App einträgst, werden NUR über st.secrets verfügbar gemacht – NICHT
# automatisch als Umgebungsvariable (os.environ). Deshalb wird hier zuerst
# st.secrets geprüft und erst danach os.environ als Fallback (z.B. für den
# lokalen CLI-Gebrauch):
#   Windows (PowerShell):  $env:SCRAPER_EMAIL = "deine@mail.at"
#   Mac/Linux:              export SCRAPER_EMAIL="deine@mail.at"
#   Streamlit Cloud:        Secrets-Tab -> SCRAPER_EMAIL = "deine@mail.at"
def _get_scraper_email():
    try:
        import streamlit as st
        if "SCRAPER_EMAIL" in st.secrets:
            return st.secrets["SCRAPER_EMAIL"]
    except Exception:
        pass
    return os.environ.get("SCRAPER_EMAIL", "")


EMAIL = _get_scraper_email()
...
if not EMAIL:
    print(
        "⚠️  Keine SCRAPER_EMAIL gefunden (weder in st.secrets noch als Umgebungsvariable). "
        "OpenAlex-Anfragen laufen dadurch über den stark rate-gelimiteten 'common pool' und "
        "können mit HTTP 429 fehlschlagen. Empfehlung: SCRAPER_EMAIL in den Streamlit-Cloud-"
        "Secrets setzen."
    )
...
# -----------------------------------------------------
def fetch_page(issn, cursor="*", retries=6):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if response.status_code == 429:
                # Rate-Limit: den vom Server vorgeschlagenen Retry-After-Header
                # respektieren, sonst großzügiges exponentielles Backoff.
                retry_after = response.headers.get("Retry-After")
                if retry_after and retry_after.isdigit():
                    wait_s = int(retry_after) + 1
                else:
                    wait_s = min(5 * (2 ** (attempt - 1)), 60)  # 5s, 10s, 20s, 40s, 60s, 60s...
                print(f"  429 Too Many Requests (Versuch {attempt}/{retries}) – warte {wait_s}s...")
                if attempt < retries:
                    time.sleep(wait_s)
                    continue
                raise RuntimeError(
                    f"Abruf für ISSN {issn} scheitert dauerhaft an Rate-Limits (HTTP 429) "
                    f"nach {retries} Versuchen. Setze SCRAPER_EMAIL (siehe oben), um den "
                    f"'polite pool' von OpenAlex zu nutzen, und/oder versuche es später erneut."
                )
...
def scrape_journal(journal_code, progress_callback=None):
    ...
    while True:
        ...
        # kleine, höfliche Pause zwischen Seiten, um Rate-Limits gar nicht
        # erst zu provozieren (v.a. relevant ohne gesetzte SCRAPER_EMAIL)
        time.sleep(0.2 if EMAIL else 1.0)
